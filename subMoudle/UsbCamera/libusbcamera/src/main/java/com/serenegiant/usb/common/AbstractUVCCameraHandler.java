...
//			thread.handleStartRecording((String)msg.obj);
	...
	public  static final class CameraThread extends Thread {
		...
		private static int previewType=UVCCamera.PIXEL_FORMAT_YUV420SP;
...
//		private boolean isAudioThreadStart;
		...
		CameraThread(final Class<? extends AbstractUVCCameraHandler> clazz,
			...
			final float bandwidthFactor,final int previewType_) {
...
//		}
		...
		public  List<Size>  getSupportedSizeList(){
			if (mUVCCamera == null ) return null;
		...
		private boolean  startPreviewSize(){
			try {
				Log.e(TAG,"startPreviewSize....mfps:"+mfps);
				mUVCCamera.setPreviewSize(mWidth, mHeight, 1, mfps,mPreviewMode , mBandwidthFactor);
				// 获取USB Camera预览数据，使用NV21颜色会失真
				// 无论使用YUV还是MPEG，setFrameCallback的设置效果一致
//				mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_YUV420SP);
				mUVCCamera.setFrameCallback(mIFrameCallback, previewType);
			} catch (final IllegalArgumentException e) {
				Log.e(TAG,"handleStartPreview..error111:"+e.toString());
				e.printStackTrace();

				try {
					// fallback to YUV mode
					mUVCCamera.setPreviewSize(mWidth, mHeight, 1, mfps, UVCCamera.DEFAULT_PREVIEW_MODE, mBandwidthFactor);
				} catch (final IllegalArgumentException e1) {
					Log.e(TAG,"handleStartPreview..error222:"+e.toString());
					e.printStackTrace();
					callOnError(e1);
					if(count==0) {
						mfps=31;
		...
		public void handleStartPreview(final Object surface) {
			...
			Log.e(TAG,"handleStartPreview.setPreviewSize.."+mWidth+"*"+mHeight+" : "+mUVCCamera.getSupportedSize());
			//mPreviewMode
			count=0;
			startPreviewSize();
...
//				Log.e("","mIFrameCallback...onFrame....ByteBuffer");
