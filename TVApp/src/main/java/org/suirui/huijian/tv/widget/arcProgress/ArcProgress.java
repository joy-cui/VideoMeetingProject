package org.suirui.huijian.tv.widget.arcProgress;
...
public class ArcProgress extends ProgressBar {
    ...
    private final float DEFAULT_mTickWidth = 2f;
    ...
    private final int DEFAULT_mStartColor = Color.BLUE;
    private final int DEFAULT_mEndColor = Color.GREEN;
    ...
    private ArgbEvaluator mArgbEvaluator = new ArgbEvaluator();//颜色渐变插值器
    private boolean isGradual = false; //是否渐变
    private Object mStartColor;
    private Object mEndColor;
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mStartColor = attributes.getColor(R.styleable.ArcProgress_startColor, DEFAULT_mStartColor);
        mEndColor = attributes.getColor(R.styleable.ArcProgress_endColor, DEFAULT_mEndColor);
        mTickWidth = attributes.getFloat(R.styleable.ArcProgress_tickWidth, DEFAULT_mTickWidth);
        ...
        isGradual = attributes.getBoolean(R.styleable.ArcProgress_isGradual, false);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        canvas.save();
        Log.e("onDraw", "getProgress == " + getProgress() + "   getMax() == " + getMax());
        ...
        if (mStylePogress == STYLE_ARC) {//shixin
            float targetmDegree = (360 - mDegree) * roate;
            if (isGradual) {
                int currentDegree = getProgress() * 360 / 100;
                //绘制未完成部分
                for (int j = currentDegree; j < 360; j++) {
                    mArcPaint.setColor(mUnmProgressColor);
                    canvas.drawArc(mArcRectf, 90 + j, mTickWidth, false, mArcPaint); // 绘制圆弧 1.35f是每个色块宽度
                }
                for (int i = 0; i < currentDegree; i++) {
                    Integer color = (Integer) mArgbEvaluator.evaluate(i / 360f, mStartColor, mEndColor);//颜色插值器（level 11以上才可以用）
                    mArcPaint.setColor(color);
                    canvas.drawArc(mArcRectf, 90 + i, mTickWidth, false, mArcPaint); // 绘制圆弧 1.35f是每个色块宽度
